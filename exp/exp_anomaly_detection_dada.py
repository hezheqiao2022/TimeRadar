...
class Exp_Anomaly_Detection_DADA(Exp_Basic):
    def __init__(self, args):
        super(Exp_Anomaly_Detection_DADA, self).__init__(args)

    def _build_model(self):
        # one for directly extract weights
        model_path = f"./{self.args.model}"
    ...
    def vali(self, vali_data, vali_loader, criterion, flag='train'):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(vali_loader):
                ...
                outputs = self.model.module.model.infer(batch_x, norm=self.args.norm)
    ...
    def pretrain(self, setting):
        ...
        for epoch in range(self.args.pretrain_epochs):
            ...
            for i, (batch_x, batch_y) in enumerate(train_loader): 
                ...
                # outputs = self.model.infer(batch_x, norm=self.args.norm)
                outputs = self.model(batch_x, norm=self.args.norm)
                loss = criterion(outputs, batch_x)
    ...
    def train(self, setting, train=0):
        ...
        # frozen
        for param in self.model.module.model.parameters():
            param.requires_grad = False  
        for param in self.model.module.model.decoder.parameters():
